...
const Navigation = () => {
  ...
  return (
    <nav className="w-[18%] h-screen bg-white shadow-md fixed left-0 top-0 p-8 flex flex-col gap-8">
      <h1 className="font-semibold text-3xl text-blue-600"> ShopFizz.app </h1>
      ...
      <h1 className="text-xl font-semibold text-blue-700">Category</h1>
      ...
      {(pathname != "/" || search.length > 0) && (
        <Link
          to={`/`}
          className={`px-3 py-3 bg-red-400 w-[40%] text-center text-white font-semibold rounded-full`}
        >
          {" "}
          Go Home{" "}
        </Link>
